...
fn main() {
    ...
    println!(" -> Item.text {:?}", root.query("List/{ns1}Items/{ns1}Item[{ns2}id=4]").one().unwrap().text());
    
    
    println!("Default namespace");
    println!(" -> Item.text {:?}", root.query("List[name=entities]").one().unwrap().query(("ns1", "Items/Item[{ns2}id=5]")).one().unwrap().text());

    println!("Query slice, and attribute filter");
    for item in root.query([("", "List[name=items]"), ("ns1", "Items"), ("ns1", "Item[tagged]")].as_slice()).all() {
        println!(" -> Item[tagged] {:?}", item.get_attr("tagged").unwrap())
    }

    println!("Attribute filter inversion");
    for item in root.query([("", "List[name=items]"), ("ns1", "Items"), ("ns1", "Item[!tagged]")].as_slice()).all() {
        println!(" -> Item[tagged] {:?}", item.get_attr("tagged").unwrap_or("<not defined>"))
    }
