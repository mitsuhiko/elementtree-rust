...
use super::{AsQueryRule, AttributeFilter, Element, IsMatch, QName};
...
fn test_query_from_str_ns_attr() {
    ...
    assert_eq!(q.name, QName::from_ns_name(Some("ns"), "item"));
    assert_eq!(
        q.filter.expect("filter not parsed").name,
        QName::from_ns_name(None, "tag")
    );
}
...
fn test_query_from_str_ns_notattr() {
    ...
    assert_eq!(q.name, QName::from_ns_name(Some("ns"), "item"));
    assert_eq!(
        q.filter.clone().expect("filter not parsed").name,
        QName::from_ns_name(None, "tag")
    );
...
fn test_query_from_str_ns_nsattr() {
    ...
    assert_eq!(
        q.filter.clone().expect("filter not parsed").name,
        QName::from_ns_name(Some("ns"), "tag")
    );
}
...
fn test_query_from_str_ns_notnsattr_value() {
    ...
    assert_eq!(
        q.filter
            .clone()
            .expect("filter not parsed")
            .value
            .expect("value not parsed"),
        "123"
    );
    assert!(q.filter.expect("filter not parsed").invert);
}
