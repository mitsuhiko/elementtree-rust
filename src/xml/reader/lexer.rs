...
use crate::xml::common::{is_name_char, is_whitespace_char, Position, TextPosition};
use crate::xml::reader::Error;
use crate::xml::util;
...
pub type Outcome = result::Result<Option<Token>, Error>;
...
impl Lexer {
    ...
    /// internals.
    #[inline]
    pub fn inside_comment(&mut self) {
        self.inside_comment = true;
    }

    /// Disables the effect of `inside_comment()` method.
    #[inline]
    pub fn outside_comment(&mut self) {
    ...
    pub fn next_token<B: Read>(&mut self, b: &mut B) -> Outcome {
        ...
        match self.st {
            State::TagStarted
            | State::CommentOrCDataOrDoctypeStarted
            | State::CommentStarted
            | State::CDataStarted(_)
            | State::DoctypeStarted(_)
            | State::CommentClosing(ClosingSubstate::Second)
            | State::DoctypeFinishing(_) => Err(self.error("Unexpected end of stream")),
            State::ProcessingInstructionClosing => Ok(Some(Token::Character('?'))),
            State::EmptyTagClosing => Ok(Some(Token::Character('/'))),
            State::CommentClosing(ClosingSubstate::First) => Ok(Some(Token::Character('-'))),
            State::CDataClosing(ClosingSubstate::First) => Ok(Some(Token::Character(']'))),
            State::CDataClosing(ClosingSubstate::Second) => Ok(Some(Token::Chunk("]]"))),
    ...
    #[inline]
    fn read_next_token(&mut self, c: char) -> Outcome {
    ...
    fn dispatch_char(&mut self, c: char) -> Outcome {
        match self.st {
    ...
    #[inline]
    fn move_to(&mut self, st: State) -> Outcome {
        self.st = st;
        Ok(None)
    }

    #[inline]
    fn move_to_with(&mut self, st: State, token: Token) -> Outcome {
        self.st = st;
        Ok(Some(token))
    }

    #[inline]
    fn move_to_with_unread(&mut self, st: State, cs: &[char], token: Token) -> Outcome {
        self.char_queue.extend(cs.iter().cloned());
        self.move_to_with(st, token)
    }

    fn handle_error(&mut self, chunk: &'static str, c: char) -> Outcome {
    ...
    /// Encountered a char
    fn normal(&mut self, c: char) -> Outcome {
        match c {
    ...
    /// Encountered '<'
    fn tag_opened(&mut self, c: char) -> Outcome {
        match c {
            ...
            _ if is_whitespace_char(c) => {
                self.move_to_with_unread(State::Normal, &[c], Token::OpeningTagStart)
            }
            _ if is_name_char(c) => {
                self.move_to_with_unread(State::Normal, &[c], Token::OpeningTagStart)
            }
    ...
    /// Encountered '<!'
    fn comment_or_cdata_or_doctype_started(&mut self, c: char) -> Outcome {
        match c {
    ...
    /// Encountered '<!-'
    fn comment_started(&mut self, c: char) -> Outcome {
        match c {
    ...
    /// Encountered '<!['
    fn cdata_started(&mut self, c: char, s: CDataStartedSubstate) -> Outcome {
        use self::CDataStartedSubstate::{CDa, CDat, CData, C, CD, E};
    ...
    /// Encountered '<!D'
    fn doctype_started(&mut self, c: char, s: DoctypeStartedSubstate) -> Outcome {
        use self::DoctypeStartedSubstate::{Do, Doc, Doct, Docty, Doctyp, D};
    ...
    /// Encountered '?'
    fn processing_instruction_closing(&mut self, c: char) -> Outcome {
        match c {
    ...
    /// Encountered '/'
    fn empty_element_closing(&mut self, c: char) -> Outcome {
        match c {
    ...
    /// Encountered '-'
    fn comment_closing(&mut self, c: char, s: ClosingSubstate) -> Outcome {
        match s {
    ...
    /// Encountered ']'
    fn cdata_closing(&mut self, c: char, s: ClosingSubstate) -> Outcome {
        match s {
            ClosingSubstate::First => match c {
...
mod tests {
    ...
    fn make_lex_and_buf(s: &str) -> (Lexer, BufReader<Cursor<Vec<u8>>>) {
        (
            Lexer::new(),
            BufReader::new(Cursor::new(s.to_owned().into_bytes())),
        )
    }
    ...
    fn simple_lexer_test() {
        let (mut lex, mut buf) =
            make_lex_and_buf(r#"<a p='q'> x<b z="y">d	</b></a><p/> <?nm ?> <!-- a c --> &nbsp;"#);
    ...
    fn special_chars_test() {
        let (mut lex, mut buf) = make_lex_and_buf(r#"?x!+ // -| ]z]]"#);
    ...
    #[test]
    fn cdata_test() {
        let (mut lex, mut buf) = make_lex_and_buf(r#"<a><![CDATA[x y ?]]> </a>"#);
    ...
    #[test]
    fn doctype_test() {
        let (mut lex, mut buf) = make_lex_and_buf(r#"<a><!DOCTYPE ab xx z> "#);
        assert_oks!(for lex and buf ;
            Token::OpeningTagStart
            Token::Character('a')
            Token::TagEnd
    ...
    fn doctype_with_internal_subset_test() {
        let (mut lex, mut buf) = make_lex_and_buf(r#"<a><!DOCTYPE ab[<!ELEMENT ba> ]> "#);
        assert_oks!(for lex and buf ;
            Token::OpeningTagStart
            Token::Character('a')
            Token::TagEnd
    ...
    fn issue_98_cdata_ending_with_right_bracket() {
        let (mut lex, mut buf) = make_lex_and_buf(r#"<![CDATA[Foo [Bar]]]>"#);
