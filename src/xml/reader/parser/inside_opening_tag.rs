...
use crate::xml::common::is_name_start_char;
use crate::xml::namespace;
use crate::xml::reader::lexer::Token;
use crate::xml::reader::parser::{
    OpeningTagSubstate, ParserOutcome, PullParser, QualifiedNameTarget, State,
};

impl PullParser {
    pub fn inside_opening_tag(&mut self, t: Token, s: OpeningTagSubstate) -> Option<ParserOutcome> {
        ...
        match s {
                ...
                match name.prefix_ref() {
                    ...
                    _ => {
                        ...
                        match token {
                            ...
                            Token::Whitespace(_) => this.as_state_continue(State::InsideOpeningTag(OpeningTagSubstate::InsideTag)),
                            _ => unreachable!()
                        }
                    }
                }
            ...
            OpeningTagSubstate::InsideTag => match t {
                ...
                Token::Character(c) if is_name_start_char(c) => {
                    self.buf.push(c);
                    self.as_state_continue(State::InsideOpeningTag(OpeningTagSubstate::InsideAttributeName))
                }
            ...
            OpeningTagSubstate::InsideAttributeName => self.read_qualified_name(t, QualifiedNameTarget::Attribute, |this, token, name| {
                ...
                match token {
                    Token::Whitespace(_) => this.as_state_continue(State::InsideOpeningTag(OpeningTagSubstate::AfterAttributeName)),
                    Token::EqualsSign => this.as_state_continue(State::InsideOpeningTag(OpeningTagSubstate::InsideAttributeValue)),
                    _ => unreachable!()
                }
            }),
            ...
            OpeningTagSubstate::AfterAttributeName => match t {
                ...
                Token::EqualsSign => self.as_state_continue(State::InsideOpeningTag(OpeningTagSubstate::InsideAttributeValue)),
                _ => unexpected_token!(t)
            },
                    ...
                    match name.prefix_ref() {
                        ...
                        Some(namespace::NS_XMLNS_PREFIX) => {
                            ...
                            if ln == namespace::NS_XMLNS_PREFIX {
                                ...
                                this.as_state_continue(State::InsideOpeningTag(OpeningTagSubstate::InsideTag))
                            }
                        }
