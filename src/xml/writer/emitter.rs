...
use crate::xml::escape::{escape_str_attribute, escape_str_pcdata};
use crate::xml::name::{Name, OwnedName};
use crate::xml::namespace::{
    NamespaceStack, NS_EMPTY_URI, NS_NO_PREFIX, NS_XMLNS_PREFIX, NS_XML_PREFIX,
};

use crate::xml::writer::config::EmitterConfig;
...
pub struct Emitter {
    ...
    indent_level: usize,
    indent_stack: Vec<WroteIndentation>,
...
impl Emitter {
    pub fn new(config: EmitterConfig) -> Emitter {
        Emitter {
            ...
            indent_level: 0,
            indent_stack: vec![WroteIndentation::Nothing],
...
impl Emitter {
    ...
    fn wrote_text(&self) -> bool {
        *self.indent_stack.last().unwrap() == WroteIndentation::Text
    }

    #[inline]
    fn wrote_markup(&self) -> bool {
        *self.indent_stack.last().unwrap() == WroteIndentation::Markup
    }

    #[inline]
    fn set_wrote_text(&mut self) {
        *self.indent_stack.last_mut().unwrap() = WroteIndentation::Text;
    }
    ...
    fn before_start_element<W: Write>(&mut self, target: &mut W) -> Result<()> {
        ...
        self.indent_stack.push(WroteIndentation::Nothing);
        Ok(())
    }
    ...
    fn emit_start_element_initial<W>(
        &mut self,
        target: &mut W,
        name: Name<'_>,
    ...
    pub fn emit_attributes<W: Write>(
        &mut self,
        target: &mut W,
        attributes: &[Attribute<'_>],
    ) -> Result<()> {
    ...
    pub fn emit_end_element<W: Write>(
        &mut self,
        target: &mut W,
        name: Option<Name<'_>>,
    ) -> Result<()> {
        ...
        if let Some(name) = owned_name.as_ref().map(|n| n.borrow()).or(name) {
            if self.config.normalize_empty_elements && self.just_wrote_start_element {
                self.just_wrote_start_element = false;
                let termination = if self.config.pad_self_closing {
                    " />"
                } else {
                    "/>"
                };
